...
// Fetch all active Adventure packages grouped by category
$packagesQuery = $conn->prepare("SELECT * FROM packages WHERE category_type='Adventure' AND status IN (1,2) ORDER BY FIELD(package_category,'Budget','Mid-Range','Premium')");
...
<head>
    <meta charset="UTF-8">
    <title>Ceylon Panorama | Adventure Packages</title>
...
<body>
    ...
    <section class="hero d-flex flex-column" style="background-image: url('css/Adventuremain.png');">
        ...
        <div class="container text-center text-white d-flex flex-column justify-content-center align-items-center flex-grow-1">
            <h1 class="font-weight-bold">Unleash Your Inner Explorer</h1>
            <p class="lead">Thrills, Spills, and Unforgettable Adventures</p>
        </div>
    </section>
                ...
                <div class="packages-container">
                    <?php foreach ($packs as $row): ?>
                        <div class="package-card">
                            ...
                            <?php else: ?>
                                <a href="signin.php?redirect=adventure.php" class="btn-add-cart">Sign In to Book</a>
                            <?php endif; ?>
                        </div>
                    <?php endforeach; ?>
                </div>
